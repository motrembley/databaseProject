...
-- #11 Gives the Hospital name, city and state where the hospital is based (Tokyo right now)
SELECT h_name, l_city, l_state FROM Hospitals, Location 
    WHERE l_city LIKE '%Tokyo%' AND h_address LIKE '%Tokyo%'
...
-- #13 Gives the name and best Hospital choice from those needed to be admitted from the given location
SELECT DISTINCT p_name, r_caseID, h_name 
    FROM (SELECT p_name, r_caseID, nh_severity, nh_descision 
        FROM Person, NeedsHospital, Results 
        WHERE nh_address LIKE '%London%' AND p_address LIKE '%London%' AND p_ID = r_personID) as t1, Hospitals 
...
-- #19 gives the number of positive cases and total test results
SELECT pos, tot FROM (SELECT COUNT(r_caseID) as pos FROM Results WHERE r_result = 'POSITIVE'), (SELECT COUNT(r_caseID) as tot FROM Results)
